...
{
    public abstract class Bomba : Entidad, ILanzable
    {
        ...
        // este metodo permite anotar a quien lanzo la bomba
        // para avisarle, cuando haya pasado el tiempo, de que exploto
        ...
        public virtual void Explotar()
        {
            if (!this.FueDestruido() & this.PasoElTiempo())
            {
                ...
                if(lanzador != null)    // esta linea es para hacer pasar las pruebas solamente, ya que siempre la bomba va a explotar porque alguien la coloco
                    this.lanzador.NotificarExplosion();
            }
        }
        ...
        public void RetardoAdquirido(double porcentajeDeRetardo)
        {
